...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Use best checkpoint to visualize classification results on val+test.")
    parser.add_argument("--config", required=True, help="训练使用的配置 JSON。")
    parser.add_argument("--metadata", default=None, help="训练生成的 metadata JSON，默认自动推断。")
    parser.add_argument("--checkpoint", default=None, help="使用的模型权重，默认 metadata 中 best_checkpoint。")
    parser.add_argument("--symbol", default="QQQ", help="绘图标的，默认 QQQ。")
                ...
                records.append(
                    {
                        ...
                        "prob": prob_val,
                        "pred_label": float(prob_val >= threshold),
                        "split": split_name,
                    }
                )
...
def _plot_candles(ax: plt.Axes, df: pd.DataFrame) -> None:
    ...
    for idx, row in df.iterrows():
        ...
        ax.add_patch(Rectangle((idx - candle_width / 2, lower), candle_width, height, edgecolor=color, facecolor=color))
...
def plot_classification_results(
    ...
    fig, (ax_actual, ax_pred) = plt.subplots(2, 1, figsize=(16, 10), sharex=True)

    # Actual labels panel
    _plot_candles(ax_actual, merged)
    for idx, label in enumerate(merged["actual_label"]):
        color = "green" if label >= 0.5 else "red"
        ax_actual.axvspan(idx - 0.5, idx + 0.5, color=color, alpha=0.1)
    ax_actual.set_title(f"{symbol} Val+Test Actual Labels")
    ax_actual.set_ylabel("Price")
    ax_actual.grid(True, linestyle="--", alpha=0.3)

    # Predicted labels panel
    _plot_candles(ax_pred, merged)
    for idx, label in enumerate(merged["pred_label"]):
        color = "green" if label >= 0.5 else "red"
        ax_pred.axvspan(idx - 0.5, idx + 0.5, color=color, alpha=0.1)
    ax_pred.plot(positions, merged["prob"], label="Predicted Prob.", color="#1f77b4", linewidth=1.5)
    ax_pred.axhline(0.5, color="#555555", linestyle="--", linewidth=1)
    ax_pred.set_title(f"{symbol} Val+Test Predicted Labels")
    ax_pred.set_ylabel("Price / Prob")
...
def main() -> None:
    ...
    for split in available_splits:
        ...
        if len(loader.dataset) == 0:
            continue
        split_pred = gather_split_predictions(model, loader, device, split, args.threshold)
    ...
    pred_symbol = pred_df[pred_df["symbol"] == target_symbol].copy()
    if pred_symbol.empty:
        raise ValueError(f"预测结果中不存在标的 {target_symbol}。")

    merged = price_df.merge(pred_symbol[["date", "prob", "pred_label"]], on="date", how="inner")
    merged = merged.sort_values("date").reset_index(drop=True)
    if merged.empty:
        raise ValueError("价格数据与预测无法对齐，请检查数据处理流程。")

    accuracy = float((merged["actual_label"].round().astype(int) == merged["pred_label"].astype(int)).mean())
    ...
    plot_classification_results(merged, target_symbol, output_path)
    print(f"[Metrics] Val+Test Accuracy={accuracy:.4f}")
