...
def prepare_feature_dataframe(feature_mode: str, symbols: Sequence[str] | None = None) -> pd.DataFrame:
    ...
    for symbol in symbol_list:
        ...
        dataset = feature_df.join(trend_df[["trend_tvalue"]], how="inner")
        dataset = dataset.dropna(axis=0, subset=["trend_tvalue"])
        dataset = dataset.sort_index()
        dataset = dataset.reset_index().rename(columns={"index": "date"})
        dataset["symbol"] = symbol
        dataset["label"] = dataset["trend_tvalue"].astype(float)
