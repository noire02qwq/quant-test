...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train transformer for experiment-1005")
    parser.add_argument("--config", type=str, default="experiment-1005/configs/transformer_alpha158.json")
    ...
    output_root = Path(paths_cfg.get("output_dir", "experiment-1005/outputs")).resolve()
    ...
    print(f"[experiment-1005] Total symbols available: {len(all_symbols)}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"[experiment-1005] Epoch {epoch}: processing {len(epoch_symbols)} symbols")
        ...
        for idx, symbol in enumerate(epoch_symbols, start=1):
            ...
            if len(ds_train) == 0 or len(ds_val) == 0 or len(ds_test) == 0:
                print(f"[experiment-1005] Skipping {symbol} due to insufficient data")
                continue
            ...
            if idx % symbol_checkpoint_interval == 0:
                ...
                if current_scaler is not None and current_feature_cols is not None:
                    ...
                    print(
                        f"[experiment-1005] Epoch {epoch} checkpoint after {idx} symbols: "
