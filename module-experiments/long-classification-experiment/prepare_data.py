...
class DatasetConfig:
    ...
    symbol: str = "QQQ"
    start: str = "2014-07-01"  # 向前补 6 个月缓冲，覆盖 2015-01-01
    end_exclusive: str = "2025-07-01"  # 向后补 6 个月缓冲，确保 2024-12-31 可用
    force_refresh: bool = False


def _ensure_dirs(force_refresh: bool) -> None:
    """确保原始数据与 qlib 输出目录按需刷新。"""
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="准备 long-classification-experiment 的原始及 qlib 数据集")
    parser.add_argument("--refresh", action="store_true", help="重新下载并构建数据（会覆盖现有文件）")
    return parser.parse_args(argv)


def main(argv: Iterable[str] | None = None) -> None:
    """下载 QQQ 行情并输出 qlib 数据集。"""
