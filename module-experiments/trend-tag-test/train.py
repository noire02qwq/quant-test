...
LABEL_MAP = TREND_CLASS_MAP


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train trend-scanning multi-class model")
...
def main() -> None:
    ...
    base_cols = {"label", "symbol", "date"}
    label_related = {col for col in train_frame.columns if col.startswith("trend_")}
    exclude_cols = base_cols | label_related
    feature_cols = [col for col in train_frame.columns if col not in exclude_cols]

    model_cfg = config.get("model", {})
    model_name = model_cfg.get("name", "transformer")
    model_params = model_cfg.get("params", {})
    num_classes = int(model_params.pop("num_classes", 3))
    ...
    feature_cols = [col for col in scaled_frames["train"].columns if col not in exclude_cols]
    ...
    if best_epoch > 0:
        print(f"[Result] Best val acc {best_val_acc:.4f} at epoch {best_epoch}")
    ...
    metadata = {
        ...
        "class_distribution": {int(k): float((frame["trend_class"] == k).mean()) for k in LABEL_MAP.keys()},
        "scaler_path": str(scaler_path.resolve()),
        "best_checkpoint": str(best_path.resolve()) if best_path else "",
        "splits": splits,
    }
    if "trend_tvalue" in frame.columns:
        positives = frame.loc[frame["trend_class"] == 1, "trend_tvalue"]
        negatives = frame.loc[frame["trend_class"] == -1, "trend_tvalue"]
        if not positives.empty:
            metadata["positive_threshold"] = float(positives.min())
        if not negatives.empty:
            metadata["negative_threshold"] = float(negatives.max())
