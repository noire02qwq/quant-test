...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train trend t-value regression model")
...
def train_epoch(
    ...
    for inputs, targets in loader:
        ...
        mse_total += loss.item() * batch_count
        mae_total += mae_fn(preds.detach(), targets).item()
        count += batch_count
    return mse_total / count, mae_total / count
...
def main() -> None:
    ...
    feature_mode = config.get("feature_mode", "raw_alpha158")
