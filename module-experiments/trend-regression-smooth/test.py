...
def plot_kline_with_regression(
    ...
    value_column = "trend_ret_pct"
    if isinstance(values, pd.DataFrame):
        if value_column not in values.columns:
            raise KeyError(f"DataFrame must contain '{value_column}' column for regression plot.")
        value_df = values[["date", value_column]].copy()
    else:
        value_df = values.to_frame(name=value_column).copy()
    ...
    ax_value.plot(positions, df[value_column], color="#1f77b4", label=line_label)
    ax_value.axhline(0.0, color="#555555", linestyle="--", linewidth=1)
    ax_value.set_ylabel("Return (%)")
...
def main() -> None:
    ...
    feature_mode_params = metadata.get("feature_mode_params", {})
    ...
    for split_name in target_splits:
        ...
        if split_name == "test":
            ...
            for symbol, price_df in split_frame.groupby("symbol"):
                ...
                if values.empty:
                    continue
                true_series = values.rename(columns={"true_value": "trend_ret_pct"})[["date", "trend_ret_pct"]]
                pred_series = values.rename(columns={"pred_value": "trend_ret_pct"})[["date", "trend_ret_pct"]]
                plot_kline_with_regression(
                    price_df,
                    true_series,
                    symbol,
                    f"{symbol} Test Period - True Return",
                    run_dir / f"{symbol}_test_true_ret.png",
                    line_label="True Return (%)",
                )
                plot_kline_with_regression(
                    price_df,
                    pred_series,
                    symbol,
                    f"{symbol} Test Period - Predicted Return",
                    run_dir / f"{symbol}_test_pred_ret.png",
                    line_label="Pred Return (%)",
                )
                print(
                    f"[Plot] {symbol} test charts saved: "
                    f"{symbol}_test_true_ret.png, {symbol}_test_pred_ret.png"
                )
