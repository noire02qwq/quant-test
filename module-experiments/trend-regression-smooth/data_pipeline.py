...
from trend_tag_analysis import TrendScanParams, compute_trend_scanning_labels
...
def prepare_feature_dataframe(
    ...
    mode_params: Mapping[str, object] | None = None,
) -> pd.DataFrame:
    symbol_list = list(symbols) if symbols else list(DEFAULT_SYMBOLS)
    params = TrendScanParams()
    frames: List[pd.DataFrame] = []
    extra_params = dict(mode_params or {})
    alpha_kind_default = extra_params.get("alpha_kind")
    r2_threshold = float(extra_params.get("min_r2", 0.75))
    ...
    for symbol in symbol_list:
        ...
        if mode in {"alpha", "raw_alpha158", "raw_alpha360"}:
            if mode == "alpha":
                alpha_kind = str(alpha_kind_default or "alpha158").lower()
            else:
        ...
        label_cols = ["trend_ret_pct", "trend_tvalue", "trend_window", "trend_r2", "trend_target_date"]
        available = [col for col in label_cols if col in trend_df.columns]
        dataset = feature_df.join(trend_df[available], how="inner")
        dataset = dataset.dropna(axis=0, subset=["trend_ret_pct"])
        if "trend_r2" in dataset.columns:
            dataset = dataset[dataset["trend_r2"] >= r2_threshold]
        if dataset.empty:
            continue
        dataset = dataset.sort_index()
        dataset = dataset.reset_index().rename(columns={"index": "date"})
        dataset["symbol"] = symbol
        dataset["label"] = dataset["trend_ret_pct"].astype(float)
