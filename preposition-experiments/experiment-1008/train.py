...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train regression model for preposition-experiments/experiment-1008")
...
def main() -> None:
    ...
    feature_cols = [col for col in train_frame.columns if col != "label"]
    scaler = fit_scaler(train_frame, feature_cols)
    save_scaler(output_dir / "scaler.json", scaler)

    scaled_frames = {name: apply_scaler(df, scaler) for name, df in frames.items()}
    feature_cols = [col for col in scaled_frames["train"].columns if col != "label"]
    ...
    metadata = {
        ...
        "scaler_path": str((output_dir / "scaler.json").resolve()),
...
# python preposition-experiments/experiment-1008/train.py --config preposition-experiments/experiment-1008/configs/transformer_traditional.json
