# preposition-experiments/experiment-1003: Transformer 时序分类独立实验
...
## 数据与特征
- 输入 CSV：`config.json` 的 `data_path` 指向 `preposition-experiments/experiment-1003/tsm_20140101_20241231.csv`。
- 特征：
...
## 训练与评估
1. 训练与验证（保存最优模型到 `preposition-experiments/experiment-1003/outputs/models/best.pt`）：
   ```bash
   python preposition-experiments/experiment-1003/train.py
   ```
2. 验证集评估：
   ```bash
   python preposition-experiments/experiment-1003/validate.py
   ```
3. 测试集评估：
   ```bash
   python preposition-experiments/experiment-1003/test.py
   ```
